...
met = "met_genPt>=200&&GenSusyMNeutralino==270&&GenSusyMStop==300"
#Ht (Just the visible Jets (every single Jet_pt > 30 and abs(Jeteta) < 2.4)) - Sum has to be bigger than 300
ht = "Sum$(GenJet_pt*(GenJet_pt>30&&abs(GenJet_eta)<2.4&&GenSusyMNeutralino==270&&GenSusyMStop==300))>300"
#ISR JET (at least one jet with pt above 100 and abs(Jeteta) < 2.4) 
ISR = "Sum$(GenJet_pt>100&&abs(GenJet_eta)<2.4&&GenSusyMNeutralino==270&&GenSusyMStop==300)>0"  #Wieso muss ich  hier eine Summe bilden und dann groesser 0 - geht ohne nicht auch? - Ws werden Events doppelt gezaehlt? zB bei zwei Jets mit pt > 100 in einem Event werden diese doppelt angenommen? 
#Just one lepton in the event with lepton pt > 20 pt
lep20 = "Sum$((abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13)&&GenPart_pt>20&&GenPart_pt<30&&abs(GenPart_eta)<2.4&&GenSusyMNeutralino==270&&GenSusyMStop==300)<2"
#Lepton pt between 5 GeV and 30 GeV
lep = "Sum$((abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13)&&GenPart_pt>5&&GenPart_pt<30&&abs(GenPart_eta)<2.4&&GenSusyMNeutralino==270&&GenSusyMStop==300)>0"
