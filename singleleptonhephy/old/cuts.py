...
#CT1  
met = "met_genPt>=100"
Ct1 = "min(met_genPt,Sum$(GenJet_pt*(GenJet_pt>30))-100)>200"
#
##ISR JET (at least one jet with pt above 100 and abs(Jeteta) < 2.4) 
ISR = "Sum$(GenJet_pt>100&&abs(GenJet_eta)<2.4)>0"  
...
#vetotau = "Sum$(abs(GenPart_pdgId)==5)==0"
###Singlelepton
###Just one additional  lepton in the event with lepton: 5 < pt < 20
#lep = "Sum$(abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13&&GenPart_pt>5&&abs(GenPart_eta)<2.4)>0"#&&GenSusyMNeutralino==270&&GenSusyMStop==300"
#
#
#
#
###lep20 = "Sum$((abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13)&&GenPart_pt>5&&GenPart_pt<=20&&abs(GenPart_eta)<2.4)<2"
###Lepton pt between 5 GeV and 25 GeV
###lep = "Sum$((abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13)&&GenPart_pt>5&&GenPart_pt<25&&abs(GenPart_eta)<2.4)>0"
##
##
###--------------------------------------------------------------------
###Section SR1
...
#etaSR1 = "abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13&&abs(GenPart_eta)<1.5"
#lepSR1 = "abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13&&GenPart_pt<30"
#chargeSR1 = "abs(GenPart_pdgId)==11||abs(GenPart_pdgId)==13&&GenPart_charge<0"
...
#bottom = "Sum$(abs(Jet_partonFlavour)==5&&Jet_pt<60&&Jet_pt>30&&abs(Jet_eta)<2.4&&GenSusyMNeutralino==270&&GenSusyMStop==300)>0"
...
#put together
cuts = [susy,met,Ct1,ISR,dphi]#,ISR,dphi,vetohard,vetotau,lep,ctSR1,etaSR1,lepSR1,chargeSR1]#,vetotau,lep20,lep] #,dphi]
cut = "&&".join(cuts)
